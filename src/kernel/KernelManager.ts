...
import type { KernelState, KernelOutput, ConnectionInfo } from '../types';
import { DATAFRAME_MIME_TYPE } from '../types';
...
export class KernelManager extends EventEmitter {
  ...
  constructor(pythonPath: string, workingDir?: string) {
    super();
  ...
  private handleIopubMessage(msg: JupyterMessage): void {
    ...
    if (this.protocol!.isStream(msg)) {
      ...
      // Prioritize rich formats - DataFrame MIME type has highest priority
      // to ensure pandas DataFrames render with our interactive viewer
      if (data[DATAFRAME_MIME_TYPE]) {
        emitOutput({
          type: 'display',
          content: JSON.stringify(data[DATAFRAME_MIME_TYPE]),
          mimeType: DATAFRAME_MIME_TYPE,
        });
